...
Cachebox is a self-hosted cache server with an HTTP API, raw-byte values,
cache metadata, stampede protection, tag invalidation, bounded memory, metrics,
and AI-oriented helper utilities.

Start here:

- [Quickstart](quickstart.md): run the server and make first requests.
- [Usage Guide](usage.md): examples for keys, TTLs, stale values, batch reads,
  tags, leases, memory limits, metrics, and errors.
- [Native Sockets](native-sockets.md): start TCP and Unix socket native
  listeners and use the binary codec from Rust.
