...
The harness starts native loopback servers on random local ports and Unix
socket paths, opens persistent native TCP and native Unix socket connections,
performs warmup requests, then measures each scenario for a fixed duration. It
reports:

- `p50_ns`, `p95_ns`, and `p99_ns`: sampled request latency percentiles.
- `throughput_ops_s`: completed benchmark operations per second.
- `memory_used_bytes`: current memory accounting when available.
- `cost_score_total`: current cost-score accounting when available.
